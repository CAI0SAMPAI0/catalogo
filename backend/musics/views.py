...
class MusicCreateSchema(Schema):
    name: str = Field(..., min_length=1, max_length=200, description="Nome da música")
    description: str | None = None
    type: str = Field(
        ..., min_length=1, max_length=100, description="Tipo da obra musical (Single, Álbum, etc.)"
    )
    ...
    def validate_name(cls, value: str) -> str:
        if not value.strip():
            raise ValueError(
                "O nome da música não pode ser vazio ou conter apenas espaços."
            )
        return value.strip()
...
class ReviewCreateSchema(Schema):
    ...
    comment: str | None = Field(
        None, max_length=500, description="Comentário opcional sobre a música"
    )
