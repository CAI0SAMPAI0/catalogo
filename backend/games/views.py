...
class GameCreateSchema(Schema):
    ...
    type: str = Field(
        ..., min_length=1, max_length=100, description="Tipo/Gênero do jogo"
    )
    ...
    def validate_name(cls, value: str) -> str:
        if not value.strip():
            raise ValueError(
                "O nome do jogo não pode ser vazio ou conter apenas espaços."
            )
        return value.strip()
...
class ReviewCreateSchema(Schema):
    ...
    comment: str | None = Field(
        None, max_length=500, description="Comentário opcional sobre o jogo"
    )
